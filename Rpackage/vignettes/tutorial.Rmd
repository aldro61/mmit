...
This document is an short overview of the functionalities of the mmit package.
...
### Splitting algorithm
As proposed in the paper, the splitting is done using a margin-based hinge loss, which yields a sequence of convex optimization problems. A dynamic programming algorithm is used to compute the optimal solution to all of these problems in log-linear time. Hence obtaining the optimal splitting value.
...
The pruning of the tree is done using mmit.pruning() where minimum cost complexity pruning is implemented.
...
`mmit.cv(target.mat, feature.mat, param_grid, n_folds = 3, scorer = NULL, pruning = TRUE, seed = NULL)`
...
result <- mmit.cv(target.mat, feature.mat, param_grid, scorer = mse, seed = 1)
```
...
library(mmit)
  ...
  target.mat <- neuroblastomaProcessed$target.mat[1:45,]
  if(require(future)){ plan(multiprocess)}
  trees <- mmif(target.mat, feature.mat, max_depth = Inf, margin = 2.0, loss = "hinge", min_sample = 1, seed = 1)
```
...
`mmif.cv(target.mat, feature.mat, param_grid, n_folds = 3, scorer = NULL, seed = NULL)`
...
if(require(future)){ plan(multiprocess)}
result <- mmif.cv(target.mat, feature.mat, param_grid, scorer = mse, seed = 1)
```
...
#lapply(result$best_estimator, plot)

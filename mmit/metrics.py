...
def zero_one_loss(y_true, y_pred):
    ...
    """
    return 1.0 * sum(0 if (t[0] <= p <= t[1]) else 1 for p, t in zip(y_pred, y_true)) / len(y_pred)
...
def mean_squared_error(y_true, y_pred):
    ...
    """
    return 1.0 * sum((p - t[0])**2 if p < t[0] else ((p - t[1])**2 if t[1] < p else 0.0) for p, t in zip(y_pred, y_true)) / len(y_pred)